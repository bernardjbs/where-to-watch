...
let passedMovieObject = {
    ...
    movieImage: "https://m.media-amazon.com/images/M/MV5BMTcyNTEyOTY0M15BMl5BanBnXkFtZTgwOTAyNzU3MDI@._V1_Ratio0.6837_AL_.jpg", 
    movieImdbRating: 7,
    ...
    movieGoodReviews: [
        {
            ...
            reviewerScore: 10,
            reviewerComment: "Lorem ipsum, dolor sit amet consectetur adipisicing elit. Commodi, consequatur. Voluptates nihil est aliquam dolor. Commodi sapiente nobis fugiat officia provident magni. Eveniet expedita doloremque voluptatum veniam repellat consectetur illum."
        },
        {
...
const fetchAllData = () => {
    ...
    fetch(`${IMDBurl}Title/${IMDBapiKey}/${movieID}`).then(function(response) {return response.json()}).then(function(imdbTitleData) {
        ...
        fetch(`${IMDBurl}Reviews/${IMDBapiKey}/${movieID}`).then(function(response){return response.json()}).then(function(reviewInfo){
            ...
            reviewsPassed.forEach(review => {
                ...
                if (pos === 0 && neg > 0) {
                    if (review.rate < 5 && review.rate != undefined && review.rate != "") {
                        ...
                        neg--;
                    } else if (review.rate < 8 && review.rate != undefined && review.rate != "") {
                        let reviewObj = {}
                        reviewObj.reviewerName = review.username;
                        reviewObj.reviewerComment = review.content;
                        reviewObj.reviewerScore = review.rate;
                        ...
                        movieBadReviews.push(reviewObj);
                        neg--;
                    } else if (review.rate < 10 && review.rate != undefined && review.rate != "") {
                        let reviewObj = {}
                        reviewObj.reviewerName = review.username;
                        reviewObj.reviewerComment = review.content;
                        reviewObj.reviewerScore = review.rate;
            ...
            let reviewsArray = [movieGoodReviews, movieBadReviews];
            let positive = 2;
            reviewsArray.forEach(review => {
                review.forEach(subReview => {
        
                    if (subReview.reviewerScore > 6 && positive > 0) {
                    ...
                    } else if (subReview.reviewerScore < 5) {
                    ...
                    } else if (subReview.reviewerScore < 8) {

                        negativeReviews.append($(`<article class="review">`).append($(`<p>Reviewer: ${subReview.reviewerName}, Rating: ${subReview.reviewerScore}/10</p><q>${subReview.reviewerComment}</q>`)));

                    } else if (subReview.reviewerScore < 10) {
        ...
        // Use Main 3 Cast
        let passedCast = [imdbTitleData.actorList[0].id, imdbTitleData.actorList[1].id, imdbTitleData.actorList[2].id];
        passedCast.forEach(actor => {

            // Fetch each actor from The Movie Database
            fetch(`${DBurl}find/${actor}?api_key=${DBapiKey}&language=en-US&external_source=imdb_id`).then(function(response){return response.json()}).then(function(actorData) {
                let pTag = $(`<p id="${actorData.person_results[0].id}">`);
                pTag.text(actorData.person_results[0].name);
                movieTopActors.append(pTag);
                let divTag = $(`<div data-movies-hidden="true" id="${actorData.person_results[0].id}_movieList"></div>`);

                // Fetch movie credits by person
                fetch(`${DBurl}person/${actorData.person_results[0].id}/movie_credits?api_key=${DBapiKey}&language=en-US`).then(function(response){ return response.json()}).then(function(movieCreds) {
                    ...
                    movieCredsObj.latestMovies.sort((a, b) =>  new Date(b.release_date) - new Date(a.release_date));
                    
                    // Get latest three movies
                    movieCredsObj.latestMovies = movieCredsObj.latestMovies.slice(0, 3);
                    ...
                    movieCredsObj.latestMovies.forEach( object => {
                        ...
                        divTag.append(`<p id="${object.titleID}">${object.title}: ${object.character}</p>`);
                    ...
                    $(`#${actorData.person_results[0].id}`).on('click', function() {
                        actorsLatestFilms.children().attr('data-movies-hidden', 'true');
                        let attr = $(`#${actorData.person_results[0].id}_movieList`).attr('data-movies-hidden');
                        attr === "false" ? 
                        $(`#${actorData.person_results[0].id}_movieList`).attr('data-movies-hidden', "true") : 
                        $(`#${actorData.person_results[0].id}_movieList`).attr('data-movies-hidden', "false");
                    })
                })
            })
        }) 
        ...
        fetch(`${WMurl}${imdbTitleData.id}/sources/?apiKey=${WMapiKey}`).then(function(response){return response.json()}).then(function(streamData) {
            ...
            streamServices.forEach(streamSite => {
                whereToStream.append($(`<h5>${streamSite.streamSvcName}: <span id="${IDvariable}${streamSite.streamSvcID}"></span></h5>`));
                let sentence = '';
